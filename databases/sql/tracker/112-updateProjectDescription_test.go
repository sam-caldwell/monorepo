...
func TestSqlDbFunc_updateProjectDescription(t *testing.T) {
	...
	const (
		...
		iconType            = "image/png"
		functionName        = "updateProjectDescription"
	...
	sqldbtest.VerifyFunctionStructure(t, db,
		...
		fmt.Sprintf("fn:%s,pn:{projectId,descriptiveText},pt:{text,uuid},rt:int4",
			strings.ToLower(functionName)))
	...
	t.Run("get project", func(t *testing.T) {
		...
		rows, err = db.Query("select updateProjectDescription('%v','%v');", projectId, newDescription)
		if err != nil {
	...
	t.Run("verify", func(t *testing.T) {
		var err error
		var rows *sql.Rows
		rows, err = db.Query("select description from projects where id='%s';", projectId)
		if err != nil {
			t.Fatalf("Fail: function call failed: %v", err)
		}
		defer func() { _ = rows.Close() }()
