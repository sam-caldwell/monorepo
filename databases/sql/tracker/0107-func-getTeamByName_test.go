...
func TestSqlDbFunc_getTeamByName(t *testing.T) {
	const (
		...
		expectedFirstName   = "Enrique"
		expectedLastName    = "Fermi"
		expectedEmail       = "enrique.fermi@example.com"
		expectedPhone       = "314.153.9246"
		expectedDescription = "Test description"
		expectedTeamName    = "Chicago Pile"
	)
	...
	t.Run("getTeamByName()", func(t *testing.T) {
		var rows *sql.Rows
		var err error
		if rows, err = db.Query("select getTeamByName('%s');", expectedTeamName); err != nil {
			t.Fatal(err)
		}
		if !rows.Next() {
			t.Fatal("no row returned")
