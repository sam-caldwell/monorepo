...
func TestSqlDbFunc_updateTicketTypeName(t *testing.T) {
	...
	const (
		...
		iconType             = "image/png"
		functionName         = "updateTicketTypeName"
	...
	t.Run("verify", func(t *testing.T) {
		var err error
		var rows *sql.Rows
		rows, err = db.Query("select name from ticketTypes where id='%s';", ticketTypeId)
		if err != nil {
		...
		if actual != expectedTicketType {
			t.Fatalf("name mismatch.  Got '%s'", actual)
		}
	})
	t.Run("call createTicketType();", func(t *testing.T) {
		var err error
		var rows *sql.Rows
		rows, err = db.Query("select updateTicketTypeName('%v','%v');", ticketTypeId, newTicketType)
		if err != nil {
			t.Fatalf("Fail: function call failed: %v", err)
		}
		defer func() { _ = rows.Close() }()
	...
	t.Run("verify", func(t *testing.T) {
		...
		if err = rows.Scan(&actualName); err != nil {
			t.Fatalf("Failed to scan rows. %v", err)
		}
		if actualName != newTicketType {
			t.Fatalf("description mismatch.  Got %s", actualName)
		}
	})
}
