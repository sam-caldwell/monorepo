...
func TestSqlDbFunc_updateProjectTeamId(t *testing.T) {
	const (
		...
		iconType            = "image/png"
		functionName        = "updateProjectTeamId"
	...
	sqldbtest.VerifyFunctionStructure(t, db,
		...
		fmt.Sprintf("fn:%s,pn:{projectId,newTeamId},pt:{uuid},rt:int4",
			strings.ToLower(functionName)))
	...
	newTeamId = createTeam(t, db, "newTeam", iconId, originalOwnerId, pRead, pRead, pRead,
		expectedDescription)
	...
	t.Run("get project", func(t *testing.T) {
		...
		rows, err = db.Query("select updateProjectTeamId('%v','%v');", projectId, newTeamId)
		if err != nil {
	...
	t.Run("verify", func(t *testing.T) {
		...
		if newTeamId != actual {
			t.Fatalf("newTeamId permission mismatch.  Got %s", newTeamId)
		}
	})
}
