...
func TestSqlDbFunc_updateTicketTypeWorkflowId(t *testing.T) {
	...
	const (
		...
		iconType             = "image/png"
		functionName         = "updateTicketTypeWorkflowId"
		...
		expectedWorkflowName = "NavigationProcess1"
		newWorkflowName      = "NavigationProcess2"
	...
	newWorkflowId = createWorkflow(t, db, newWorkflowName, iconId, ownerId, teamId,
		pRead, pRead, pRead, expectedDescription)
	...
	t.Run("verify", func(t *testing.T) {
		var err error
		var rows *sql.Rows
		rows, err = db.Query("select workflowId from ticketTypes where id='%s';", ticketTypeId)
		if err != nil {
		...
		if actual != workflowId.String() {
			t.Fatalf("workflowId mismatch.  Got %s", actual)
		}
	})
	t.Run("call createTicketType();", func(t *testing.T) {
		var err error
		var rows *sql.Rows
		rows, err = db.Query("select updateTicketTypeWorkflowId('%v','%v');", ticketTypeId, newWorkflowId)
		if err != nil {
			t.Fatalf("Fail: function call failed: %v", err)
		}
		defer func() { _ = rows.Close() }()
	...
	t.Run("verify", func(t *testing.T) {
		...
		if actual != newWorkflowId.String() {
			t.Fatalf("workflowId mismatch.  Got %s", actual)
		}
	})
}
