...
func TestSqlDbFunc_getProjectsByOwnerId(t *testing.T) {
	...
	const (
		...
		iconType            = "image/png"
		functionName        = "getProjectsByOwnerId"
	...
	sqldbtest.VerifyFunctionStructure(t, db,
		...
		fmt.Sprintf("fn:%s,pn:{thisownerid},pt:{uuid},rt:jsonb", strings.ToLower(functionName)))
	...
	t.Run("get project", func(t *testing.T) {
		...
		rows, err = db.Query("select getProjectsByOwnerId('%v');", ownerId)
		if err != nil {
