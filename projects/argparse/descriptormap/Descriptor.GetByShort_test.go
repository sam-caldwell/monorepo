...
func TestMap_GetByShort(t *testing.T) {
	var m Map
	...
	verifyRows := func(name, short, long string, typ types.ArgTypes, required bool, dValue any, help string) {
		...
		failIf(d.GetShort() != short, "short mismatch: %s (%s)", name, short)
		failIf(d.GetLong() != long, "long mismatch: %s (%s)", name, long)
		failIf(d.GetType() != typ, "type mismatch: %s (%s)", name, typ.String())
	}
