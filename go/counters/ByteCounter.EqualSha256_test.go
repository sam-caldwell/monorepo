...
func TestByteCounter_EqualSha256(t *testing.T) {
	const (
		size    = 100
	...
	t.Run("ensure the two counters return the same hash", func(t *testing.T) {
		// Ensure that EqualSha256 returns true for equal content
		if !lhs.EqualSha256(&rhs) {
			t.Error("Expected EqualSha256 to return true, but got false")
		}
	})

	t.Run("ensure the two counters return the same hash and the operation is commutative", func(t *testing.T) {
		// Ensure that EqualSha256 returns true for equal content
		if !rhs.EqualSha256(&lhs) {
			t.Error("Expected EqualSha256 to return true, but got false")
		}
	})
	...
	t.Run("Modify RHS and confirm hash equality fails", func(t *testing.T) {
		...
		// Ensure that EqualSha256 returns false for different content
		if lhs.EqualSha256(&rhs) {
			t.Error("Expected EqualSha256 to return false, but got true")
		}
	})
	...
	t.Run("Modify LHS in the same way and confirm hash equality is restored", func(t *testing.T) {
		...
		// Ensure that EqualSha256 returns false for different content
		if lhs.EqualSha256(&rhs) {
			t.Error("Expected EqualSha256 to return false, but got true (lhs:rhs)")
		}
		if rhs.EqualSha256(&lhs) {
			t.Error("Expected EqualSha256 to return false, but got true (rhs:lhs)")
		}
	})
}
