...
func main() {
    ...
    go func() {
        ...
        for continueRunning {
            ...
            case <-t.C:
                ...
                log.Printf("generator progres: %12d/%12d (%8.2f %%) [%6d] gOps:%8.2f "+
                    "storage: %12d/%12d (%8.2f %%) [%6d] elapsed: %v",
                    genCount, PreComputeSize, gProgress, genDuration.Milliseconds(), gOps,
                    storeCount, len(queue), sProgress, storeDuration.Milliseconds(), elapsed.Seconds())
            }
        }
    }()
    ...
    go func() {
        ...
        for continueRunning {
            storeStart := time.Now()
            if _, err := db.Exec("INSERT INTO hashes (h) VALUES ($1),($2),($3),($4);", <-queue, <-queue, <-queue, <-queue); err != nil {
                log.Fatal(err)
            }
            storeCount++
