...
func TestRandomInt64(t *testing.T) {
	t.Run("happy path: test multiples", func(t *testing.T) {
		minInput, maxInput := int64(-10), int64(100)
		numSamples := 1000
		for i := 0; i < numSamples; i++ {
			random := Int64(minInput, maxInput)
	...
	t.Run("happy path: test range", func(t *testing.T) {
		minInput, maxInput := int64(math.MinInt64), int64(math.MaxInt64)
		numSamples := 1000
		for i := 0; i < numSamples; i++ {
			random := Int64(minInput, maxInput)
			if random < minInput || random >= maxInput {
	...
	t.Run("Test when max is less than or equal to min", func(t *testing.T) {
		minInput, maxInput := int64(math.MaxInt64), int64(math.MinInt64)
		random := Int64(minInput, maxInput)
		if random != 0 {
			t.Fatalf("Int64(%d, %d) = %d; want 0 as max is less than or equal to min", minInput, maxInput, random)
		}
	})
}
