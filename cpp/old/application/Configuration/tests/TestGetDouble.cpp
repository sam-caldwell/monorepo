/**
 * @name Configuration/tests/TestGetDouble.cpp
...
class TestGetDouble : public TestBase {
private:
    Configuration *cfg;
    ConfigStateMap *map;
public:
    ...
    /**
     * @name test_get_double
     * @brief expect that given a known ConfigStateMap record the getDouble() method will return the corresponding
     *        ValueTypes value.
     *
     * @return bool (pass/fail)
     */
    bool test_get_double() {
        return expect(cfg->getDouble("internal3") == 1.0, "Expect .getDouble(internal3) will return 1.0.");
    }

    /**
     * @name test_get_double_missing
     * @brief expect that an out_of_range exception will be thrown if a non-existent key name is passed to getDouble()
     *
     * @return bool (pass/fail)
     */
    bool test_get_double_missing() {
        try {
            return expect(cfg->getDouble("BadToken") == 1.0,
                          "Expect getDouble() exception on non-existing token. Oops");
        } catch (out_of_range &e) {
            return expect(true, "Expect getDouble() exception on non-existing token. ok");
        }
    }
    ...
    bool main() {
        ...
        return expect(map, "Expect map is initialized.") &&
               ...
               expect(test_get_double(), "Expect test_get_double() OK") &&
               expect(test_get_double_missing(), "Expect test_get_double_missing() OK");
    }
};/*end of class*/