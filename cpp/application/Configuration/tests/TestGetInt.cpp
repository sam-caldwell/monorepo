/**
 * @name Configuration/tests/TestGetInt.cpp
...
class TestGetInt : public TestBase {
private:
    Configuration *cfg;
    ConfigStateMap *map;
public:
    ...
    /**
     * @name test_get_int
     * @brief expect that given a known ConfigStateMap record the getInt() method will return the corresponding
     *        ValueTypes value.
     *
     * @return bool (pass/fail)
     */
    bool test_get_int() {
        return expect(cfg->getInt("internal5") == -10, "Expect .getInt(internal1) will return Int.");
    }

    /**
     * @name test_get_int_missing
     * @brief expect that an out_of_range exception will be thrown if a non-existent key name is passed to getInt()
     *
     * @return bool (pass/fail)
     */
    bool test_get_int_missing() {
        try {
            return expect(cfg->getInt("BadToken") == -10, "Expect getInt() exception on non-existing token. Oops");
        } catch (out_of_range &e) {
            return expect(true, "Expect getInt() exception on non-existing token. ok");
        }
    }
    ...
    bool main() {
        ...
        return expect(map, "Expect map is initialized.") &&
               ...
               expect(test_get_int(), "Expect test_get_int() OK") &&
               expect(test_get_int_missing(), "Expect test_get_int_missing() OK");
    }
};/*end of class*/