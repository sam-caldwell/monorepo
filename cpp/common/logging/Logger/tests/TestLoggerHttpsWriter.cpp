/**
 * @name Logger/tests/TestLoggerHttpsWriter.cpp
...
#include "../../../../old/application/Configuration/src/Configuration.h"
#include "../src/Logger.h"
#include "../../../../Tester/Mocks/MockFile.h"

class TestLoggerHttpsWriter : public TestBase {
private:
...
public:
     ...
     * @param n
     */
    TestLoggerHttpsWriter(string n) {
        name = n;
        thisWriter = Logging::Writer::https;
     ...
     */
    ~TestLoggerHttpsWriter() {}

    /**
     * @name validateLine
     * @brief Test and expect that the line is properly formatted, etc.
     *
     * @params sev - log line severity
     * @params tags - tag string
     * @params line - log message
     * @params msg - isolated message content
     * @return bool
     */
    bool validateLine(Logging::Severity sev2, string *tags, string *line, string *msg) {
        string s = Logging::ReverseSeverity.at(sev2);
        std::regex pattern(
                "^\\{\"t\": [0-9]{10}, "\
                "\"c\": [0-9]{1,3}, "\
                "\"s\":\"" + (s) + "\", "\
                "\"m\":\"" + (*msg) + "\", "\
                "\"tags\":\"" + (*tags) + "\"\\}$");
        return expect(std::regex_match(*line, pattern), "Expect log line to match regex pattern:" + *line);
    }
...
//
//        if (expect(writeOutcome == writeFlag, "Expected writeOutcome ok") && writeFlag) {
//            mock.captureLastLine();
//            string output = mock.last();
//
//            bool validate = writeFlag ? validateLine(sev2, &tags, &output, &msg) : true;
//            bool validLength = writeFlag && (output.length() == length);
//            return expect(validLength, "expect log has " + to_string(output.length()) +
//                                       " chars.  expected: " + to_string(length) + " chars: " + output) &&
//                   expect(validate, "expect output matches regex pattern.");
//        } else {
            return true;
//        }
    }
