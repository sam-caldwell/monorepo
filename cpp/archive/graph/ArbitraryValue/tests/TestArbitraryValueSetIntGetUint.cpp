/**
 * @name ArbitraryValue/tests/TestArbitraryValueSetIntGetUint.cpp
...
class TestArbitraryValueSetIntGetUint : public TestBase {
public:
    /**
     ...
     */
    TestArbitraryValueSetIntGetUint(string n) { name = n; }

    /**
     * @name Class destructor
     * @brief clean up
     */
    ~TestArbitraryValueSetIntGetUint() {}
     ...
     */
    bool main() {
        return expect(test_func(0,0),"Given int(0), expect int(0)") &&
               expect(test_func(1,1),"Given int(1), expect int(1)") &&
                expect(test_func(INT_MAX,INT_MAX),"Given int(INT_MAX), expect int(INT_MAX)") &&
                expect(test_func(INT_MIN,INT_MIN),"Given int(INT_MIN), expect int(INT_MIN)") &&
               expect(test_strong_type(0), "Expect strong type enforcement (0) ok.");
    }
};