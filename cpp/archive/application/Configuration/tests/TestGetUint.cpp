/**
 * @name Configuration/tests/TestGetUint.cpp
...
class TestGetUint : public TestBase {
private:
    Configuration *cfg;
    ConfigStateMap *map;
public:
    ...
    /**
     * @name test_get_uint
     * @brief expect that given a known ConfigStateMap record the getUint() method will return the corresponding
     *        ValueTypes value.
     *
     * @return bool (pass/fail)
     */
    bool test_get_uint() {
        return expect(cfg->getUint("internal7") == 25, "Expect .getUint(internal1) will return Uint.");
    }

    /**
     * @name test_get_uint_missing
     * @brief expect that an out_of_range exception will be thrown if a non-existent key name is passed to getUint()
     *
     * @return bool (pass/fail)
     */
    bool test_get_uint_missing() {
        try {
            return expect(cfg->getUint("BadToken") == 25, "Expect getUint() exception on non-existing token. Oops");
        } catch (out_of_range &e) {
            return expect(true, "Expect getUint() exception on non-existing token. ok");
        }
    }
    ...
    bool main() {
        ...
        return expect(map, "Expect map is initialized.") &&
               ...
               expect(test_get_uint(), "Expect test_get_uint() OK") &&
               expect(test_get_uint_missing(), "Expect test_get_uint_missing() OK");
    }
};/*end of class*/